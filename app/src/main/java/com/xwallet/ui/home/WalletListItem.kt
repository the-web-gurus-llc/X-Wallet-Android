...
import com.xwallet.legacy.presenter.customviews.ShimmerLayout
    ...
    class ViewHolder(
        ...
        override fun bindView(item: WalletListItem, payloads: List<Any>) {
            ...
            if (currencyCode.isBrd() && !BRSharedPrefs.getRewardsAnimationShown()) {
                (containerView as com.xwallet.legacy.presenter.customviews.ShimmerLayout).startShimmerAnimation()
            } else {
                (containerView as com.xwallet.legacy.presenter.customviews.ShimmerLayout).stopShimmerAnimation()
            }
        ...
        private fun setBackground(wallet: Wallet, context: Context) {
            ...
            if (wallet.isSupported) {
                // Create gradient if 2 colors exist.
                    if(wallet.currencyId == "bitcoin-mainnet:__native__") {
                        val startColor = Color.parseColor("#0791ff" ?: return)
                        val endColor = Color.parseColor("#0791ff" ?: return)
                    ...
                    } else if(wallet.currencyId == "ethereum-mainnet:__native__") {
                        val startColor = Color.parseColor("#FF5454" ?: return)
                        val endColor = Color.parseColor("#FF5454" ?: return)
                        (drawable as GradientDrawable).colors = intArrayOf(startColor, endColor)
                        drawable.orientation = GradientDrawable.Orientation.LEFT_RIGHT
                        wallet_card.background = drawable
                        setWalletItemColors(R.dimen.token_background_no_alpha)
